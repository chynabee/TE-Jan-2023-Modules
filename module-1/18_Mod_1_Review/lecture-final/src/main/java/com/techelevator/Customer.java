...
public class Customer {
    ...
    //list cons: if we  want to find something, we have to loop
    // through our list to find it
    //NOTE: ALWAYS INITIALIZE YOUR LIST OR MAP RIGHT AWAY TO AVOID NULL POINTER EXCEPTIONS
    List<BankAccount> bankAccounts = new ArrayList<>();

    //map benefits: we can give a key (name) and get the account right away,
    // without having to loop
    // cons: a little more complex, doesn't keep order unless we do a tree map or
    // linked list
    Map<String, BankAccount> bankAccountMap = new TreeMap<>();
    ...
    public void addBankAccountToMap(BankAccount account){
